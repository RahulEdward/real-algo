...
class OrderSchema(Schema):
    ...
    pricetype = fields.Str(
        load_default="MARKET", validate=validate.OneOf(["MARKET", "LIMIT", "SL", "SL-M"])
    )
    product = fields.Str(load_default="MIS", validate=validate.OneOf(["MIS", "NRML", "CNC"]))
    price = fields.Float(
        load_default=0.0, validate=validate.Range(min=0, error="Price must be a non-negative number.")
    )
...
class OptionsOrderSchema(Schema):
    ...
    product = fields.Str(
        load_default="MIS", validate=validate.OneOf(["MIS", "NRML"])
...
class MarginPositionSchema(Schema):
    ...
    price = fields.Str(load_default="0")  # String to match API contract
    trigger_price = fields.Str(load_default="0")  # String to match API contract
