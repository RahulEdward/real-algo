# Mapping RealAlgo API Request https://realalgo.in/docs
# AliceBlue does not provide Margin Calculator API

from utils.logging import get_logger

logger = get_logger(__name__)


def transform_margin_positions(positions):
    """
    Transform RealAlgo margin position format to broker format.

    Note: AliceBlue does not provide a margin calculator API.

    Args:
        positions: List of positions in RealAlgo format

    Raises:
        NotImplementedError: AliceBlue does not support margin calculator API
    """
    raise NotImplementedError("AliceBlue does not support margin calculator API")


def parse_margin_response(response_data):
    """
    Parse broker margin calculator response to RealAlgo standard format.

    Note: AliceBlue does not provide a margin calculator API.

    Args:
        response_data: Raw response from broker margin calculator API

    Raises:
        NotImplementedError: AliceBlue does not support margin calculator API
    """
    raise NotImplementedError("AliceBlue does not support margin calculator API")
