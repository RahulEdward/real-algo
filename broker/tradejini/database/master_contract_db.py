        ...
        for item in scrip_data:
            try:
                ...
                if len(parts) >= 3:
                    ...
                    # Map exchange for RealAlgo format
                ...
                if group_name == "Securities":
                    ...
                    if len(parts) >= 4:
                        ...
                        try:
                            ...
                            realalgo_symbol = f"{base_symbol}{expiry_formatted}FUT"
                    ...
                    # Get RealAlgo exchange
                    realalgo_exchange = exchange_map.get(raw_exchange, raw_exchange)

                    # Check if this is a common index symbol
                    if realalgo_exchange == "BSE_INDEX":
                        # Handle BSE indices
                        if item["symbol"].upper() in COMMON_INDEX_MAP[realalgo_exchange]["common"]:
                            ...
                            item["symbol"].upper()
                            in COMMON_INDEX_MAP[realalgo_exchange]["sectoral"]
                        ):
                            # Use sectoral index format
                            record = {
                                "symbol": item["symbol"].upper(),  # Use uppercase symbol
                                "brsymbol": COMMON_INDEX_MAP[realalgo_exchange]["sectoral"][
                                    item["symbol"].upper()
                                ],  # Use mapped brsymbol
                                "name": item.get(
                                    "symbol", item["dispName"]
                        ...
                        # Handle NSE indices
                        if item["symbol"].upper() in COMMON_INDEX_MAP[realalgo_exchange]:
    ...
    try:
        ...
        if socketio:
            socketio.emit(
                ...
                {"status": "success", "message": "Successfully downloaded all contracts"},
            )
        return True
