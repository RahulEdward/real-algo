...
def process_zebu_nse_data(output_path):
    """
    Processes the Zebu NSE data (NSE_symbols.txt) to generate RealAlgo symbols.
    ...
    # Apply transformation for RealAlgo symbols
    def get_realalgo_symbol(broker_symbol):
        ...
        if "-EQ" in broker_symbol:
            ...
            # For other symbols (including index), RealAlgo symbol remains the same as broker symbol
            return broker_symbol

    # Update the 'symbol' column
    df["symbol"] = df["brsymbol"].apply(get_realalgo_symbol)
    ...
    # Map common NSE index symbols to RealAlgo format
    nse_index_mapping = {
        "NIFTY INDEX": "NIFTY",
    ...
    df_filtered.loc[df_filtered["exchange"] == "NSE_INDEX", "symbol"] = df_filtered.loc[
        df_filtered["exchange"] == "NSE_INDEX", "symbol"
    ].replace(nse_index_mapping)
...
def process_zebu_nfo_data(output_path):
    """
    Processes the Zebu NFO data (NFO_symbols.txt) to generate RealAlgo symbols.
...
def process_zebu_cds_data(output_path):
    """
    Processes the Zebu CDS data (CDS_symbols.txt) to generate RealAlgo symbols.
    Handles both futures and options formatting.
    """
...
def process_zebu_mcx_data(output_path):
    """
    Processes the Zebu MCX data (MCX_symbols.txt) to generate RealAlgo symbols.
    Handles both futures and options formatting.
    """
...
def process_zebu_bse_data(output_path):
    """
    Processes the Zebu BSE data (BSE_symbols.txt) to generate RealAlgo symbols.
    ...
    # Apply transformation for RealAlgo symbols (no special logic needed here)
    def get_realalgo_symbol(broker_symbol):
        return broker_symbol
...
def process_zebu_bfo_data(output_path):
    """
    Processes the Zebu BFO data (BFO_symbols.txt) to generate RealAlgo symbols and correctly extract the name column.
