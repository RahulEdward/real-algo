...
class ZebuExchangeMapper:
    """Maps between RealAlgo exchange names and Zebu exchange codes"""

    # RealAlgo to Zebu exchange mapping (same as Flattrade/Noren)
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Reverse mapping
    ZEBU_TO_REALALGO = {v: k for k, v in EXCHANGE_MAP.items()}

    @classmethod
    def to_zebu_exchange(cls, oa_exchange: str) -> str | None:
        """Convert RealAlgo exchange to Zebu exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, zebu_exchange: str) -> str | None:
        """Convert Zebu exchange to RealAlgo exchange format"""
        return cls.ZEBU_TO_REALALGO.get(zebu_exchange.upper())
