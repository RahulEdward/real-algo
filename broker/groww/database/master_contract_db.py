...
# Functions for symbol format conversion between RealAlgo and Groww formats
def format_realalgo_to_groww_symbol(symbol, exchange):
    """
    Convert RealAlgo symbol format to Groww symbol format

    Args:
        symbol (str): Symbol in RealAlgo format (e.g., AARTIIND29MAY25630CE)
        exchange (str): Exchange code (NSE, BSE, NFO, etc.)

    Returns:
        str: Symbol in Groww format (e.g., "AARTIIND 29MAY25 630 CE")
    """
    logger.info(f"Converting symbol from RealAlgo to Groww format: {{symbol}}, {exchange}")
...
def format_groww_to_realalgo_symbol(groww_symbol, exchange):
    """
    Convert Groww symbol format to RealAlgo symbol format
    ...
    Returns:
        str: Symbol in RealAlgo format (e.g., "AARTIIND29MAY25630CE")
    """
    logger.info(f"Converting symbol from Groww to RealAlgo format: {{groww_symbol}}, {exchange}")
    ...
    if exchange == "NFO":
        ...
        # If already in RealAlgo format (no spaces), return as is
        ...
        if len(parts) >= 4 and parts[-1] in ("CE", "PE"):
            ...
            # Combine into RealAlgo format: [BaseSymbol][ExpirationDate][StrikePrice][OptionType]
            # Example: AARTIIND29MAY25630CE
            realalgo_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
            logger.info(f"Converted to RealAlgo format: {realalgo_symbol}")
            return realalgo_symbol
        ...
        # For futures
            ...
            # Combine into RealAlgo format: [BaseSymbol][ExpirationDate]FUT
            # Example: NIFTY29MAY25FUT
            realalgo_symbol = f"{base_symbol}{date_str}FUT"
            logger.info(f"Converted to RealAlgo format: {realalgo_symbol}")
            return realalgo_symbol
        ...
        # Handle case where option type might be missing
            ...
            if is_date:
                try:
                    ...
                    realalgo_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
                    logger.info(
                        f"Converted to RealAlgo format (assumed {{option_type}}): {realalgo_symbol}"
                    )
                    return realalgo_symbol
                except ValueError:
...
def find_symbol_by_token(token, exchange):
    ...
    Returns:
        str: Symbol in RealAlgo format, or None if not found
    """
...
def find_token_by_symbol(symbol, exchange):
    ...
    Args:
        symbol (str): Symbol in either RealAlgo or Groww format
        exchange (str): Exchange code
    ...
    if exchange == "NFO":
        # Try with RealAlgo format if it was in Groww format
        realalgo_symbol = format_groww_to_realalgo_symbol(symbol, exchange)
        if realalgo_symbol != symbol:
            result = (
                db_session.query(SymToken)
                .filter_by(symbol=realalgo_symbol, exchange=exchange)
                .first()
            )
            if result:
                return result.token

        # Try with Groww format if it was in RealAlgo format
...
def process_groww_data(path):
    ...
    try:
        ...
        # Format the symbol for F&O (NFO) instruments to match RealAlgo format
        def format_fo_symbol(row):
            ...
            # For segment='FNO', format according to RealAlgo standard
    ...
    # Map instrument types to RealAlgo standard types
    instrument_type_map = {
        "EQUITY": "EQ",
        "INDEX": "INDEX",
        "FUTURE": "FUT",
    ...
    # Map exchanges to RealAlgo standard exchanges
    ...
    # For F&O instruments, format the symbol in RealAlgo format
    fo_mask = all_instruments["exchange"] == "NFO"
    if fo_mask.any():
        # Format F&O symbols according to RealAlgo standard
        def format_fo_symbol(row):
...
def master_contract_download():
    ...
    try:
        ...
        # Step 5: Add RealAlgo symbols where needed (converting Groww format to RealAlgo format)
        ...
        for idx, row in nfo_options.iterrows():
            # Convert the broker symbol to RealAlgo format if spaces are detected
            if " " in row["brsymbol"]:
                token_df.at[idx, "symbol"] = format_groww_to_realalgo_symbol(row["brsymbol"], "NFO")
