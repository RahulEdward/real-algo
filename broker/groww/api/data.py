...
class BrokerData:
    ...
    def _convert_realalgo_to_groww_derivative_symbol(self, symbol):
        """
        Convert RealAlgo NFO/BFO symbol format to Groww format
    ...
    def _convert_to_groww_params(self, symbol, exchange):
        ...
        if exchange in ["NFO", "BFO"]:
            ...
            if br_symbol:
                ...
                trading_symbol = self._convert_realalgo_to_groww_derivative_symbol(symbol)
    ...
    def get_history(
        ...
        try:
            ...
            if not df.empty:
                ...
                # NOTE: For RealAlgoXTS, return non-indexed DataFrame with timestamp as a column
    ...
    def get_intervals(self) -> dict[str, dict[str, list[str]]]:
        """
        Get supported timeframes for Groww historical data in the RealAlgo format.
        ...
        # Return in the standard RealAlgo format
    ...
    def get_quotes(self, symbol_list, timeout: int = 5) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Quote data in RealAlgo format
        """
        ...
        for sym in symbol_list:
            try:
                ...
                # Map RealAlgo exchange to Groww exchange format
                if exchange == "NSE":
                    groww_exchange = EXCHANGE_NSE
                    segment = SEGMENT_CASH
                elif exchange == "BSE":
                ...
                try:
                    ...
                    if response and not response.get("error"):
                        ...
                        if response.get("status") == "SUCCESS" and isinstance(
                            ...
                            # Create quote_item in RealAlgo format
                            ...
                            # CRITICAL: Build the quote item directly with values extracted from the response, using field names that RealAlgo understands
        ...
        # Single symbol case - return in simpler format for RealAlgo frontend
    ...
    def _format_single_quote_response(self, quote_data):
        """Helper method to convert from standard dict to the format expected by RealAlgo frontend
        ...
        logger.debug(f"Final RealAlgo quote format (data only): {result}")
        return result
        ...
        logger.info(f"Formatting single quote response for RealAlgo frontend: {quote}")

        # Based on the sample response, RealAlgo expects exactly these fields
        ...
        # LTP - preferred field name in RealAlgo
        ...
        # Return exact structure expected by RealAlgo
        result = {"status": "success", "data": simple_data}

        logger.info(f"FINAL FORMATTED RESULT: {result}")
        logger.info(f"Formatted result for RealAlgo frontend: {result}")
    ...
    def get_depth(self, symbol_list, timeout: int = 5) -> dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Market depth data in RealAlgo format
        """
        ...
        try:
            ...
            # Create a properly formatted response for RealAlgo
            depth_response = {}
            ...
            # Format the depth response according to RealAlgo requirements
            depth_response = {
                "bids": bids,
                "asks": asks,
                "ltp": ltp,
    ...
    def _process_quotes_batch(self, symbols: list) -> list:
        ...
        for item in symbols:
            ...
            try:
                ...
                # For derivatives, convert symbol format
                if exchange in ["NFO", "BFO"]:
                    br_symbol = self._convert_realalgo_to_groww_derivative_symbol(br_symbol)
