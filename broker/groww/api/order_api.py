...
from broker.groww.database.master_contract_db import (
    format_groww_to_realalgo_symbol,
    format_realalgo_to_groww_symbol,
)
...
def direct_get_order_book(auth):
    ...
    try:
        ...
        # Convert all symbols from Groww format to RealAlgo format
        for order in all_orders:
            if "trading_symbol" in order:
                ...
                # First, determine the correct RealAlgo exchange
                ...
                if any(suffix in groww_symbol for suffix in ["CE", "PE", "C", "P"]):
                    exchange = "NFO"
                    is_derivative = True
                    order["exchange"] = "NFO"  # Set RealAlgo exchange format
                    logger.info(
                ...
                # For NFO derivatives (options or futures), convert from Groww format to RealAlgo format
                if is_derivative:
                    ...
                    try:
                        ...
                        if token:
                            realalgo_symbol = get_oa_symbol(token, "NFO")
                            logger.info(f"RealAlgo Symbol: {realalgo_symbol}")
                            if realalgo_symbol:
                                order["symbol"] = realalgo_symbol
                                logger.info(
                                    f"Converted NFO symbol by token: {groww_symbol} -> {realalgo_symbol}"
                                )
                        ...
                        try:
                            ...
                            if not is_future:
                                ...
                                if match:
                                    ...
                                    # Format as RealAlgo expects: NIFTY15MAY2526650CE
                                    realalgo_symbol = (
                                        f"{symbol_name}{day}{month_name}{year}{strike}{option_type}"
                                    )
                                    order["symbol"] = realalgo_symbol
                                    logger.info(
                                        f"Converted Groww option symbol by pattern: {groww_symbol} -> {realalgo_symbol}"
                                    )
                                    symbol_converted = True
                            ...
                            # For Futures: Convert from "NIFTY2551FUT" to "NIFTY29MAY25FUT"
                                ...
                                if match:
                                    ...
                                    # Format as RealAlgo expects: NIFTY29MAY25FUT
                                    realalgo_symbol = f"{symbol_name}{day}{month_name}{year}FUT"
                                    order["symbol"] = realalgo_symbol
                                    logger.info(
                                        f"Converted Groww futures symbol by pattern: {groww_symbol} -> {realalgo_symbol}"
                                    )
                                    symbol_converted = True
                        except Exception as e:
...
def get_trade_book(auth):
    ...
    try:
        ...
        # Format trades to match RealAlgo's expected format (as used in the REST API)
        ...
        for trade in all_trades:
            ...
            realalgo_trade = {
                # Fields expected by RealAlgo's UI
            ...
            realalgo_trades.append(realalgo_trade)

        # Log the first transformed trade for debugging
        if realalgo_trades:
            logger.info(
                f"Sample RealAlgo trade format: {json.dumps(realalgo_trades[0], indent=2, default=str)}"
            )
        ...
        response = {
            ...
            "data": realalgo_trades,  # This is what map_trade_data will look for first
            "tradebook": realalgo_trades,  # For compatibility with different naming conventions
        ...
        try:
            ...
            if response_obj.status_code == 200 and response_data.get("status") == "SUCCESS":
                ...
                if "payload" in response_data and "positions" in response_data["payload"]:
                    ...
                    # Transform positions to match RealAlgo's expected format
                    for position in raw_positions:
                        ...
                        try:
                            ...
                            try:
                                ...
                                from realalgo.database.token_db import get_oa_symbol

                            # First try database lookup for any symbol
                            db_symbol = get_oa_symbol(groww_symbol, "NFO")
                            if db_symbol:
                                realalgo_symbol = db_symbol
                                logger.info(
                                    f"Database: Converted Groww symbol: {groww_symbol} -> {realalgo_symbol}"
                                )
                                symbol_converted = True
                            else:
                                ...
                                if option_match:
                                    ...
                                    logger.info(
                                        f"Pattern: Converted Groww option symbol: {groww_symbol} -> {realalgo_symbol}"
                                    )
                                    symbol_converted = True
                                else:
                                    ...
                                    if future_match:
                                        ...
                                        realalgo_symbol = f"{symbol_name}{day}{month_name}{year}FUT"
                                        logger.info(
                                            f"Pattern: Converted Groww futures symbol: {groww_symbol} -> {realalgo_symbol}"
                                        )
                                        symbol_converted = True
                        ...
                        # Map exchange to RealAlgo format
                        exchange = position.get("exchange", "")
                        if exchange == "NSE":
                            realalgo_exchange = "NSE_EQ"
                        elif exchange == "BSE":
                            realalgo_exchange = "BSE_EQ"
                        elif exchange == "NFO":
                            realalgo_exchange = "NSE_FO"
                        else:
                            realalgo_exchange = exchange

                        # Create position object in RealAlgo format
                        ...
                        if position.get("segment") == "CASH":
                            ...
                            position_symbol = (
                                realalgo_symbol  # Use converted symbol for other segments
                            )

                        transformed_position = {
                            # Standard RealAlgo fields
                            "symbol": position_symbol,
                            "tradingsymbol": position_symbol,
                            "exchange": realalgo_exchange,
                            ...
                            "segment": "EQ",  # RealAlgo format for CASH segment
                            # Specific Groww fields (renamed to match RealAlgo expectations)
                ...
                if fno_response.status_code == 200:
                    ...
                    if (
                        ...
                        for position in fno_positions:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    if db_symbol:
                                        realalgo_symbol = db_symbol
                                        logger.info(
                                            f"Database: Converted Groww FNO symbol: {groww_symbol} -> {realalgo_symbol}"
                                        )
                                        symbol_converted = True
                                    else:
                                        # Fallback to pattern matching if database lookup fails
                                        # For Options: Convert from Groww format to RealAlgo format
                                        # Groww format: "NIFTY25051334000CE" or "BANKNIFTY25051332500PE"
                                        # RealAlgo format: "NIFTY13MAY2534000CE" or "BANKNIFTY13MAY2532500PE"
                                        groww_pattern = re.compile(
                                            r"([A-Z]+)(\d{2})(\d{2})(\d{2})(\d+)([CP]E)"
                                        )
                                        match = groww_pattern.match(groww_symbol)
                                    ...
                                    if match:
                                        ...
                                        # Format as RealAlgo expects: NIFTY15MAY2526650CE
                                        realalgo_symbol = f"{symbol_name}{day}{month_name}{year}{strike}{option_type}"
                                        logger.info(
                                            f"Pattern: Converted Groww option position symbol: {groww_symbol} -> {realalgo_symbol}"
                                        )
                                        symbol_converted = True
                                    ...
                                    # For Futures: Convert from "NIFTY2551FUT" to "NIFTY29MAY25FUT"
                                        ...
                                        if match:
                                            ...
                                            logger.info(
                                                f"Pattern: Converted Groww futures position symbol: {groww_symbol} -> {realalgo_symbol}"
                                            )
                                            symbol_converted = True
                                except Exception as e:
                                    logger.error(f"Error converting position symbol: {e}")
                            ...
                            exchange = position.get("exchange", "")
                            if exchange == "NSE":
                                realalgo_exchange = "NSE"
                            elif exchange == "BSE":
                                realalgo_exchange = "BSE"
                            elif exchange == "NFO":
                                realalgo_exchange = "NSE_FO"
                            else:
                                realalgo_exchange = exchange
                            ...
                            transformed_position = {
                                "symbol": realalgo_symbol,
                                "tradingsymbol": realalgo_symbol,
                                "exchange": realalgo_exchange,
                                "product": position.get("product", ""),
                                "quantity": net_qty,
                                "net_quantity": net_qty,
                                ...
                                "buy_quantity": buy_qty,
                                "sell_quantity": sell_qty,
                                "segment": "FO",  # RealAlgo format for FNO segment
                ...
                if "holdings" in response_data["payload"]:
                    ...
                    for holding in raw_holdings:
                        transformed_holding = {
                            ...
                            # Using the key names RealAlgo expects
...
def get_open_position(tradingsymbol, exchange, product, auth):
    ...
    Returns:
        str: Net quantity
    """
    # Convert Trading Symbol from RealAlgo Format to Broker Format Before Search
...
def direct_place_order_api(data, auth):
    ...
    Args:
        data (dict): Order data in RealAlgo format
        auth (str): Authentication token
    ...
    try:
        ...
        if db_record and db_record.brsymbol:
            ...
            trading_symbol = format_realalgo_to_groww_symbol(original_symbol, original_exchange)
            logger.info(
        ...
        if resp.status_code == 200:
            ...
            if response_data.get("status") == "SUCCESS":
                ...
                formatted_response = {
                    ...
                    "trading_symbol": trading_symbol,
                    "symbol": original_symbol,  # Add original RealAlgo symbol to response
                }
...
def place_smartorder_api(data, auth):
    ...
    try:
        ...
        try:
            ...
            from realalgo.database.token_db import get_br_symbol
...
def get_holdings(auth):
    ...
    try:
        ...
        # Transform holdings to RealAlgo format
...
def cancel_order(orderid, auth, segment=None, symbol=None, exchange=None):
    ...
    Args:
        ...
        symbol (str, optional): Trading symbol in RealAlgo format
    ...
    """
    try:
        # If symbol is provided, convert it from RealAlgo to Groww format
        if symbol and exchange:
            groww_symbol = format_realalgo_to_groww_symbol(symbol, exchange)
        ...
        if response_obj.status_code == 200:
            ...
            # If symbol is provided, include it in RealAlgo format in the response
            if symbol:
                # Add the original RealAlgo format symbol to the response
                response["symbol"] = symbol
                logger.info(f"Including RealAlgo symbol in cancel response: {symbol}")
...
def direct_modify_order(data, auth):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # If symbol was provided in the original request, include it in RealAlgo format
            if "symbol" in data and data["symbol"]:
                response["symbol"] = data["symbol"]
                logger.info(f"Including RealAlgo symbol in modify response: {data['symbol']}")
            ...
            if order_status.upper() in [s.upper() for s in cancellable_statuses]:
                try:
                    ...
                    if (
                        ...
                        # Get and include symbol in the RealAlgo format
                        if "symbol" in order:
                            broker_symbol = order.get("symbol", "")

                            # For NFO symbols that have spaces, convert to RealAlgo format
                            ...
                            if exchange == "NFO" and " " in broker_symbol:
                                try:
                                    ...
                                    realalgo_symbol = format_groww_to_realalgo_symbol(
                                        broker_symbol, exchange
                                    )
                                    if realalgo_symbol:
                                        cancelled_item["symbol"] = realalgo_symbol
                                        cancelled_item["brsymbol"] = (
                                            broker_symbol  # Keep original broker symbol for reference
                                        )
                                        logger.info(
                                            f"Transformed cancelled order symbol for UI: {broker_symbol} -> {realalgo_symbol}"
                                        )
                                except Exception as e:
                                    logger.error(
