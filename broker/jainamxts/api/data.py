...
def get_api_response(endpoint, auth, method="GET", payload="", feed_token=None, params=None):
    ...
    headers = {"authorization": FEED_TOKEN, "Content-Type": "application/json"}
...
class BrokerData:
    def __init__(self, auth_token, feed_token=None, user_id=None):
        """Initialize JainamXTS data handler with authentication token"""
        self.auth_token = auth_token
        self.feed_token = feed_token
        self.user_id = user_id
        logger.debug(
            f"BrokerData initialized - auth_token present: {bool(auth_token)}, feed_token present: {bool(feed_token)}"
        )
    ...
    def get_history(self, symbol, exchange, timeframe, from_date, to_date):
        ...
        try:
            ...
            while current_start <= to_date:
                ...
                # JainamXTS expects MMM DD YYYY HHMMSS in IST
                ...
                if not response or response.get("type") != "success":
                    ...
                    if "Invalid Token" in error_msg:
                        logger.info(
                            "Feed token expired during historical data fetch, attempting to refresh..."
                        )
                        if self._refresh_feed_token():
                            ...
                            if not response or response.get("type") != "success":
                                ...
                                raise Exception(
                                    f"Error from JainamXTS API: {response.get('description', 'Unknown error') if response else 'No response'}"
                                )
                        else:
                            logger.error("Failed to refresh feed token")
                            raise Exception(f"Error from JainamXTS API: {error_msg}")
            ...
            if not dfs:
                if compression_value == "D" and to_date.date() == datetime.now().date():
                    ...
                    if not response or response.get("type") != "success":
                        ...
                        if "Invalid Token" in error_msg:
                            logger.info(
                                "Feed token expired during today's candle fetch, attempting to refresh..."
                            )
                            if self._refresh_feed_token():
                                response = get_api_response(
                                    "/instruments/quotes",
