...
class DhanExchangeMapper:
    """Maps between RealAlgo exchange names and Dhan exchange codes"""

    # RealAlgo to Dhan exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE_EQ",
        "BSE": "BSE_EQ",
        "NFO": "NSE_FNO",
    ...
    # Dhan exchange segment codes (numeric) to RealAlgo exchange mapping
    ...
    # Reverse mappings
    DHAN_TO_REALALGO = {v: k for k, v in EXCHANGE_MAP.items()}
    EXCHANGE_TO_SEGMENT = {v: k for k, v in SEGMENT_TO_EXCHANGE.items()}

    @classmethod
    def get_dhan_exchange(cls, realalgo_exchange: str) -> str | None:
        """Convert RealAlgo exchange to Dhan exchange format"""
        return cls.EXCHANGE_MAP.get(realalgo_exchange)

    @classmethod
    def get_realalgo_exchange(cls, dhan_exchange: str) -> str | None:
        """Convert Dhan exchange to RealAlgo exchange format"""
        return cls.DHAN_TO_REALALGO.get(dhan_exchange)

    @classmethod
    def get_exchange_from_segment(cls, segment_code: int) -> str | None:
        """Convert Dhan exchange segment code to RealAlgo exchange"""
    ...
    def get_segment_from_exchange(cls, exchange: str) -> int | None:
        """Convert RealAlgo exchange to Dhan exchange segment code"""
