...
def transform_margin_position(position, client_id):
    """
    Transform a single RealAlgo margin position to Dhan margin format.

    Note: Dhan margin calculator API accepts only one order at a time, not a batch.

    Args:
        position: Position in RealAlgo format
...
def map_product_type_for_margin(product):
    """
    Maps RealAlgo product type to Dhan product type for margin calculation.
...
def parse_margin_response(response_data):
    """
    Parse Dhan margin response to RealAlgo standard format.
...
def parse_batch_margin_response(responses):
    ...
    This is a limitation of the Dhan API, not RealAlgo.

    Args:
        responses: List of individual margin responses (one per leg)

    Returns:
        Aggregated margin response matching RealAlgo format
    """
    try:
        total_margin = 0
        total_span = 0
