...
def get_client_id(api_key=None):
    ...
    Args:
        api_key: RealAlgo API key (optional)
...
def calculate_single_margin(position_data, auth, client_id):
    ...
    try:
        ...
        logger.info("STANDARDIZED REALALGO RESPONSE")
        logger.info("=" * 80)
...
def calculate_margin_api(positions, auth, api_key=None):
    ...
    This limitation is due to Dhan API design, not RealAlgo.

    Args:
        positions: List of positions in RealAlgo format
        auth: Authentication token for Dhan
        api_key: RealAlgo API key (optional, for client ID lookup)
