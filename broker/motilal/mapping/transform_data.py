...
def map_exchange(exchange):
    """
    Maps RealAlgo exchange names to Motilal Oswal exchange names.

    RealAlgo uses: NSE, BSE, NFO, CDS, MCX, BFO
...
def reverse_map_exchange(exchange):
    """
    Reverse maps Motilal Oswal exchange names to RealAlgo exchange names.
    """
...
def transform_data(data, token):
    """
    Transforms the RealAlgo API request structure to Motilal Oswal expected structure.
    """
    symbol = get_br_symbol(data["symbol"], data["exchange"])
    realalgo_exchange = data["exchange"]
    motilal_exchange = map_exchange(realalgo_exchange)
    ...
    transformed = {
        ...
        "producttype": map_product_type(
            data["product"], realalgo_exchange
        ),  # Pass RealAlgo exchange for context
...
def transform_modify_order_data(data, token, lastmodifiedtime, qtytradedtoday):
    ...
    Args:
        data: RealAlgo modify order request data
...
def map_order_type(pricetype):
    """
    Maps RealAlgo pricetype to Motilal Oswal order type.
...
def map_product_type(product, exchange=None):
    """
    Maps RealAlgo product type to Motilal Oswal product type based on exchange.
    ...
    Args:
        product: RealAlgo product type (CNC, MIS, NRML)
        exchange: RealAlgo exchange name (NSE, BSE, NFO, CDS, MCX, BFO)
    """
    # Determine if this is a cash segment or derivative segment
    # Using RealAlgo exchange names
...
def reverse_map_product_type(product, exchange=None):
    """
    Reverse maps Motilal Oswal product type to RealAlgo product type.
    Context:
