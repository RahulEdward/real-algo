...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    # Convert Trading Symbol from RealAlgo Format to Broker Format Before Search in OpenPosition
    tradingsymbol = get_br_symbol(tradingsymbol, exchange)
    # Map exchange from RealAlgo format to Motilal format for comparison
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response.get("status") == "SUCCESS":
        ...
        for position in positions_response["data"]:
            ...
            # Convert Motilal exchange to RealAlgo exchange for symbol lookup
            motilal_exchange = position["exchange"]
            realalgo_exchange = reverse_map_exchange(motilal_exchange)

            # Get realalgo symbol to send to placeorder function
            symbol = get_symbol(position["symboltoken"], realalgo_exchange)
            logger.info(f"The Symbol is {symbol}")

            if not symbol:
                logger.error(
                    f"Symbol not found for token {position['symboltoken']} and exchange {realalgo_exchange}"
                )
                continue
            ...
            place_order_payload = {
                ...
                "symbol": symbol,
                "action": action,
                "exchange": realalgo_exchange,  # Use RealAlgo exchange format
                "pricetype": "MARKET",
                "product": reverse_map_product_type(position["productname"], realalgo_exchange),
