...
class ShoonyaExchangeMapper:
    """Maps between RealAlgo exchange names and Shoonya exchange codes"""

    # RealAlgo to Shoonya exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Reverse mapping
    SHOONYA_TO_REALALGO = {v: k for k, v in EXCHANGE_MAP.items()}

    @classmethod
    def to_shoonya_exchange(cls, oa_exchange: str) -> str | None:
        """Convert RealAlgo exchange to Shoonya exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, shoonya_exchange: str) -> str | None:
        """Convert Shoonya exchange to RealAlgo exchange format"""
        return cls.SHOONYA_TO_REALALGO.get(shoonya_exchange.upper())
