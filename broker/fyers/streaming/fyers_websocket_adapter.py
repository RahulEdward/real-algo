"""
Fyers WebSocket Adapter for RealAlgo WebSocket Proxy
Integrates with the RealAlgo WebSocket proxy system
"""
...
class FyersWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Fyers-specific implementation of the WebSocket adapter for RealAlgo proxy"""
    ...
    def __init__(self):
        ...
        self.tbt_symbol_to_ticker = {}  # RealAlgo symbol -> Fyers ticker
        self.tbt_ticker_to_symbol = {}  # Fyers ticker -> RealAlgo symbol
    ...
    def _subscribe_tbt_depth(
        ...
        Args:
            symbol: RealAlgo symbol (actual symbol without suffix)
            exchange: Exchange name
    ...
    def _unsubscribe_tbt_depth(self, symbol: str, exchange: str) -> bool:
        ...
        Args:
            symbol: RealAlgo symbol (may include :50 suffix)
            exchange: Exchange name
    ...
    def _convert_to_fyers_ticker(self, symbol: str, exchange: str) -> str | None:
        """
        Convert RealAlgo symbol to Fyers ticker format using database lookup

        Args:
            symbol: RealAlgo symbol (e.g., 'RELIANCE', 'NIFTY24DEC25000CE')
    ...
    def _on_tbt_depth_update(self, ticker: str, depth_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to RealAlgo format
            symbol = subscription["symbol"]
            exchange = subscription["exchange"]

            self.logger.debug(f"Mapping TBT depth for {exchange}:{symbol}")

            mapped_data = self.data_mapper.map_tbt_depth_to_realalgo(
    ...
    def _map_fyers_to_realalgo(
        self, fyers_data: dict[str, Any], mode: int
    ) -> dict[str, Any] | None:
        """
        Map Fyers data to RealAlgo WebSocket format
        ...
        Returns:
            Mapped data in RealAlgo format
        """
        try:
            if not fyers_data:
                return None
            ...
            if mode == 1:  # LTP
                ...
                realalgo_data.update({"ltp": converted_ltp, "data_type": "LTP"})
            elif mode == 2:  # Quote
