"""
Fyers Data Mapping
Maps Fyers HSM data to RealAlgo format for compatibility
"""
...
class FyersDataMapper:
    """
    Maps Fyers HSM WebSocket data to RealAlgo format
    """
    ...
    def map_to_realalgo_ltp(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to RealAlgo LTP format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            RealAlgo LTP format dict or None if mapping fails
        """
        try:
            ...
            return realalgo_data

        except Exception as e:
            logger.debug(f"Error mapping LTP data: {e}")
            return None

    def map_to_realalgo_quote(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to RealAlgo Quote format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            RealAlgo Quote format dict or None if mapping fails
        """
        try:
            if not fyers_data:
                return None
    ...
    def map_to_realalgo_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers depth data to RealAlgo Depth format

        Args:
            fyers_data: Raw depth data from Fyers HSM WebSocket

        Returns:
            RealAlgo Depth format dict or None if mapping fails
        """
        try:
            ...
            # Map to RealAlgo Depth format (matching other brokers)
            realalgo_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_index_to_synthetic_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers index data to synthetic RealAlgo Depth format
    ...
    def map_fyers_data(
        ...
        """
        Map Fyers data to appropriate RealAlgo format based on requested type
        ...
        Returns:
            Mapped RealAlgo data or None if mapping fails
        """
        if not fyers_data:
            return None
        ...
        if requested_type == "LTP":
            return self.map_to_realalgo_ltp(fyers_data)
        elif requested_type == "Quote":
            return self.map_to_realalgo_quote(fyers_data)
        elif requested_type == "Depth" and fyers_type == "dp":
            return self.map_to_realalgo_depth(fyers_data)
    ...
    def extract_symbol_info(self, symbol: str) -> dict[str, str]:
        """
        Extract exchange and symbol from RealAlgo format
