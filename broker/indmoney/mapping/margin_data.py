...
def transform_margin_positions(positions):
    """
    Transform RealAlgo margin position format to IndMoney margin format.

    Note: IndMoney margin API calculates margin for single orders only,
    not batch calculations like Angel Broking.

    Args:
        positions: List of positions in RealAlgo format
...
def map_segment(exchange):
    """
    Maps RealAlgo exchange to IndMoney segment.

    RealAlgo: NSE, BSE, NFO, BFO, CDS, BCD, MCX
...
def map_exchange_type(exchange):
    """
    Maps RealAlgo exchange to IndMoney exchange format.
...
def map_product_type(product):
    """
    Maps RealAlgo product type to IndMoney product type.

    RealAlgo: CNC, NRML, MIS
...
def parse_margin_response(response_data):
    """
    Parse IndMoney margin calculator response to RealAlgo standard format.

    Args:
        response_data: Raw response from IndMoney margin calculator API

    Returns:
        Standardized margin response matching RealAlgo format
    """
    try:
        ...
        # Extract only the three required values as per RealAlgo API specification
        total_margin = data.get("total_margin", 0)
        span_margin = data.get("span_margin", 0)
        exposure_margin = data.get("exposure_margin", 0)

        # Return standardized format matching RealAlgo API specification
