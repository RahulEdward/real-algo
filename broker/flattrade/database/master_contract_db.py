...
def process_flattrade_nse_data(output_path):
    """
    Processes the Flattrade NSE data (NSE_Equity.csv) to generate RealAlgo symbols.
    ...
    try:
        ...
        df["lotsize"] = (
            pd.to_numeric(df["lotsize"], errors="coerce").fillna(1).astype(int)
        )  # Default lotsize to 1
...
def process_flattrade_nfo_data(output_path):
    """
    Processes the Flattrade NFO data (NFO.csv) to generate RealAlgo symbols.
...
def process_flattrade_cds_data(output_path):
    """
    Processes the Flattrade CDS data (Currency_Derivatives.csv) to generate RealAlgo symbols.
    Handles both futures and options formatting.
    """
...
def process_flattrade_mcx_data(output_path):
    """
    Processes the Flattrade MCX data (Commodity.csv) to generate RealAlgo symbols.
    Handles both futures and options formatting.
    """
...
def process_flattrade_bse_data(output_path):
    """
    Processes the Flattrade BSE data (BSE_Equity.csv) to generate RealAlgo symbols.
    ...
    df["lotsize"] = (
        pd.to_numeric(df["lotsize"], errors="coerce").fillna(1).astype(int)
    )  # Convert to int, default to 1 like NSE
...
def process_flattrade_bfo_data(output_path):
    """
    Processes the Flattrade BFO data (BFO.csv) to generate RealAlgo symbols.
    Handles both futures and options formatting.
    """
