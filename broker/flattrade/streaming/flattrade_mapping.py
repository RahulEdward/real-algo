...
class FlattradeExchangeMapper:
    """Maps between RealAlgo exchange names and Flattrade exchange codes"""

    # RealAlgo to Flattrade exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Reverse mapping
    FLATTRADE_TO_REALALGO = {v: k for k, v in EXCHANGE_MAP.items()}

    @classmethod
    def to_flattrade_exchange(cls, oa_exchange: str) -> str | None:
        """Convert RealAlgo exchange to Flattrade exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, flattrade_exchange: str) -> str | None:
        """Convert Flattrade exchange to RealAlgo exchange format"""
        return cls.FLATTRADE_TO_REALALGO.get(flattrade_exchange.upper())
