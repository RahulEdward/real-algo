...
class KotakWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """
    Adapter for Kotak WebSocket streaming, suitable for RealAlgo or similar frameworks.
    ...
    def __init__(self):
        ...
        # Mapping from Kotak format to RealAlgo format - critical for data flow
        self._kotak_to_realalgo = {}  # {(kotak_exchange, token): (exchange, symbol)}
    ...
    def _on_data_received(self, parsed_data):
        ...
        try:
            ...
            with self._lock:
                ...
                if mapping_key in self._kotak_to_realalgo:
                    exchange, symbol = self._kotak_to_realalgo[mapping_key]
    ...
    def subscribe_quote(self, exchange, symbol, mode):
        ...
        try:
            ...
            with self._lock:
                mapping_key = (kotak_exchange, str(token))
                self._kotak_to_realalgo[mapping_key] = (exchange, symbol)
            ...
            with self._lock:
                ...
                if mapping_key in self._symbol_modes:
                    ...
                    if not self._symbol_modes[mapping_key]:
                        self._kotak_to_realalgo.pop(mapping_key, None)
