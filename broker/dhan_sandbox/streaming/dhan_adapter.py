"""
Fixed Dhan WebSocket adapter for RealAlgo.
...
from .dhan_mapping import get_dhan_exchange, get_realalgo_exchange
...
class DhanWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    Implements RealAlgo WebSocket proxy interface with proper mode mapping.
    """
    ...
    def __init__(self):
        ...
        # Extended mode mapping to handle all possible RealAlgo modes
        self.mode_map = {
            # Standard RealAlgo modes
    ...
    def subscribe(
        ...
        try:
            ...
            # Map RealAlgo mode to Dhan mode with explicit handling of all possible modes
            if mode == 1:
                # Standard LTP mode
                dhan_mode = DhanWebSocket.MODE_LTP
                self.logger.info(f"Mapped RealAlgo mode {mode} (LTP) to Dhan mode '{dhan_mode}'")
            elif mode == 2:
                # Standard QUOTE mode
                dhan_mode = DhanWebSocket.MODE_QUOTE
                self.logger.info(f"Mapped RealAlgo mode {mode} (QUOTE) to Dhan mode '{dhan_mode}'")
            elif mode == 3:
                # Standard DEPTH mode
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(f"Mapped RealAlgo mode {mode} (DEPTH) to Dhan mode '{dhan_mode}'")
            else:
                # All other modes (4-8) map to FULL/DEPTH
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(
                    f"Mapped RealAlgo mode {mode} (DEPTH/FULL) to Dhan mode '{dhan_mode}'"
                )

            # Add symbol to subscription tracking
            with self.lock:
                self.subscribed_symbols[symbol] = {
                    "exchange": exchange,  # Store original RealAlgo exchange
            ...
            # Map RealAlgo exchange to Dhan exchange code
    ...
    def _on_ticks(self, ticks: list[dict[str, Any]]):
        ...
        try:
            ...
            for tick in ticks:
                ...
                # Convert to RealAlgo format for data field
                ...
                # Normalize tick format to RealAlgo standard
    ...
    def _generate_topic(self, symbol: str, exchange: str, mode_str: str) -> str:
        ...
        Args:
            ...
            exchange: The exchange code in RealAlgo format
    ...
    def _normalize_tick(self, tick: dict[str, Any]) -> dict[str, Any]:
        """
        Normalize Dhan tick data to RealAlgo format.
        ...
        try:
            ...
            # Ensure exchange is in RealAlgo format
            exchange = tick.get("exchange")
            if exchange:
                realalgo_exchange = get_realalgo_exchange(exchange)
            else:
    ...
    def validate_tick_parsing(self, ticker_symbols=None, timeout=30):
        ...
        try:
            ...
            original_callbacks = {}
            for mode in [1, 2, 3, 4, 5]:  # RealAlgo modes
