...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    logger.info(
        f"Searching position for: symbol={tradingsymbol}, exchange={exchange}, product={producttype}"
    )
        ...
        for position in position_list:
            ...
            if pos_symbol == tradingsymbol and pos_product == producttype:
                ...
                for field in possible_qty_fields:
                    ...
                    logger.warning(
                        f"NetQty field not found. Available fields: {list(position.keys())}"
                    )
                break
...
def place_smartorder_api(data: dict, auth: str) -> tuple:
    """
    Place a smart order to achieve target position size based on the RealAlgo specification.
    ...
    try:
        ...
        if position_delta == 0:
            ...
            return (
                ...
                {
                    "status": "success",
                    "message": f"Position already matches target size of {position_size}",
                },
                None,
            )
        ...
        if order_action and order_quantity > 0:
            ...
            order_data.update({"action": order_action, "quantity": str(order_quantity)})
...
def close_all_positions(current_api_key, auth):
    ...
    for position in positions_list:
        ...
        place_order_payload = {
            ...
            "limitPrice": "0",
            "stopPrice": "0",
            "orderUniqueIdentifier": "realalgo",
        }
...
def cancel_order(orderid, auth):
    ...
    # Prepare the payload
    payload = json.dumps({"appOrderID": orderid, "orderUniqueIdentifier": "realalgo"})
