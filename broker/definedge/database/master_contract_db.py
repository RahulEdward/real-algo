...
def process_definedge_nse_csv(path):
    ...
    try:
        ...
        # Map DefinedGe NSE columns to RealAlgo schema
        # Assuming DefinedGe uses standard format: Symbol, Token, Name, etc.
        processed_df = pd.DataFrame()

        if "Symbol" in df.columns:
            processed_df["symbol"] = df["Symbol"] + "-EQ"  # RealAlgo format
...
def process_definedge_allmaster_csv(path):
    ...
    try:
        ...
        # Format symbols according to RealAlgo standard
...
def master_contract_download():
    ...
    try:
        ...
        if os.path.exists(allmaster_filepath):
            ...
            return socketio.emit(
                "master_contract_download", {"status": "error", "message": "Master file not found"}
            )
