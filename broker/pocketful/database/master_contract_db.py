...
def process_pocketful_bse_csv(path):
    ...
    token_df["exchange"] = df["segment"].map({"IDX": "BSE_INDEX"}).fillna(df["exchange"])
...
def process_pocketful_mcx_csv(path):
    ...
    # Define the function to build RealAlgo symbol
    def build_realalgo_symbol(row):
        try:
    ...
    df["symbol"] = df.apply(build_realalgo_symbol, axis=1)
