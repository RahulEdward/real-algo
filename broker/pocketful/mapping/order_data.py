...
def map_order_data(order_data):
    ...
    for order in orders:
        ...
        # Check if symbol was found; if so, update with RealAlgo format
        if symbol:
            # Convert to RealAlgo symbol format
...
def transform_order_data(orders):
    """
    Transform order data from Pocketful API format to RealAlgo standard format.
...
def map_trade_data(trade_data):
    ...
    for trade in trades:
        ...
        if symbol:
            ...
            # Convert to RealAlgo symbol format if exchange is available
            if exchange:
                oa_symbol = get_oa_symbol(symbol, exchange)
...
def transform_tradebook_data(tradebook_data):
    """
    Transform tradebook data from Pocketful API format to RealAlgo standard format.
...
def transform_holdings_data(holdings_data):
    """
    Transform holdings data from Pocketful API format to RealAlgo standard format.
    ...
    for holding in holdings_data:
        ...
        # Convert to RealAlgo symbol format if needed
        if tradingsymbol and exchange:
            ...
            # Try to convert to RealAlgo symbol format
            try:
                logger.info(
                    f"DEBUG - Converting symbol '{tradingsymbol}' for exchange '{exchange}' to RealAlgo format"
                )
                oa_symbol = get_oa_symbol(tradingsymbol, exchange)
                if oa_symbol:
                    tradingsymbol = oa_symbol
                    logger.debug(f"DEBUG - Converted to RealAlgo symbol: {tradingsymbol}")
                else:
                    logger.debug(
                        f"DEBUG - Could not convert to RealAlgo symbol, using original: {tradingsymbol}"
                    )
            except Exception as e:
                logger.error(f"DEBUG - Error converting symbol to RealAlgo format: {e}")
