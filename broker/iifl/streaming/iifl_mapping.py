...
class IiflExchangeMapper:
    """Maps between RealAlgo exchange codes and Iifl XTS specific exchange types"""

    # Exchange type mapping for Iifl XTS broker
    # Format: {RealAlgo_Exchange: Iifl_Exchange_Code}
    ...
    # Reverse mapping for converting Iifl exchange codes to RealAlgo format
    # Format: {Iifl_Exchange_Code: RealAlgo_Exchange}
    ...
    def get_exchange_type(exchange):
        """
        Convert RealAlgo exchange code to Iifl XTS specific exchange type

        Args:
            exchange: Exchange code (e.g., 'NSE', 'BSE', 'NSEFO')

        Returns:
            int: Exchange type code for Iifl XTS API
        """
        if exchange is None:
    ...
    @staticmethod
    def get_realalgo_exchange(iifl_code):
        """
        Convert Iifl XTS exchange code to RealAlgo exchange code
...
class IiflCapabilityRegistry:
    """
    Registry of Iifl XTS broker's capabilities including supported exchanges,
    subscription modes, and market depth levels
    """

    # Iifl XTS broker capabilities
