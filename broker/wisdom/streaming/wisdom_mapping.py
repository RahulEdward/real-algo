...
class WisdomExchangeMapper:
    """Maps between RealAlgo exchange codes and Wisdom XTS specific exchange types"""

    # Exchange type mapping for Wisdom XTS broker
    # Format: {RealAlgo_Exchange: Wisdom_Exchange_Code}
    ...
    # Reverse mapping for converting Wisdom exchange codes to RealAlgo format
    # Format: {Wisdom_Exchange_Code: RealAlgo_Exchange}
    ...
    def get_exchange_type(exchange):
        """
        Convert RealAlgo exchange code to Wisdom XTS specific exchange type

        Args:
            exchange: Exchange code (e.g., 'NSE', 'BSE', 'NSEFO')

        Returns:
            int: Exchange type code for Wisdom XTS API
        """
        if exchange is None:
        ...
        # Mapping based on Wisdom API documentation:
        # "NSECM": 1, "NSEFO": 2, "NSECD": 3, "BSECM": 11, "BSEFO": 12, "MCXFO": 51
        all_exchange_mappings = {
            # RealAlgo standard codes
    ...
    @staticmethod
    def get_realalgo_exchange(wisdom_code):
        """
        Convert Wisdom XTS exchange code to RealAlgo exchange code

        Args:
            wisdom_code (int): Wisdom exchange code

        Returns:
            str: RealAlgo exchange code
        """
...
class WisdomCapabilityRegistry:
    """
    Registry of Wisdom XTS broker's capabilities including supported exchanges,
    subscription modes, and market depth levels
    """

    # Wisdom XTS broker capabilities
