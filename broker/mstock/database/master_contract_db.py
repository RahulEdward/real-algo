...
def convert_date(date_str):
    """
    Convert date format to RealAlgo expiry column format: DD-MMM-YY (with hyphens).
    Example: '19MAR2024' -> '19-MAR-24' or '2024-03-19' -> '19-MAR-24'

    RealAlgo Expiry Column Format: DD-MMM-YY (e.g., 28-MAR-24, 25-APR-24)
...
def process_mstock_json(json_data):
    """
    Processes the MStock JSON data to fit the RealAlgo database schema.
    ...
    # MStock API returns NSE/BSE for currency, but RealAlgo uses CDS/BCD
    # -------------------------------------------------------------------
    ...
    mask_bse_currency = (df["instrumenttype"].isin(["OPTCUR", "FUTCUR", "OPTIRC", "FUTIRC"])) & (
        df["exchange"] == "BSE"
    )
    ...
    # Convert expiry dates to RealAlgo format (DD-MMM-YY)
    ...
    df["strike"] = (
        pd.to_numeric(df["strike"].replace("", None), errors="coerce").fillna(0).astype(float)
    )
    df["lotsize"] = (
        pd.to_numeric(df["lotsize"].replace("", None), errors="coerce").fillna(1).astype(int)
    )
    df["tick_size"] = (
        pd.to_numeric(df["tick_size"].replace("", None), errors="coerce").fillna(0.05).astype(float)
    )
    ...
    try:
        ...
        if bse_index_tokens:
            ...
            mask_bse_index = (df["exchange"] == "BSE") & (df["token"].isin(bse_index_tokens))
    ...
    # -------------------------------------------------------------------
    # Format F&O Symbols (RealAlgo Standard)
    ...
    df.loc[mask, "symbol"] = (
        df.loc[mask, "name"] + df.loc[mask, "expiry"].str.replace("-", "", regex=False) + "FUT"
    )
    ...
    mask = (
        (df["instrumenttype"].isin(["FUTCUR", "FUTIRC"]))
        & (df["exchange"] == "CDS")
        & (df["expiry"].str.len() > 0)
    )
    df.loc[mask, "symbol"] = (
    ...
    mask_ce = (
        (df["instrumenttype"].isin(["OPTCUR", "OPTIRC"]))
        & (df["exchange"] == "CDS")
        & (df["brsymbol"].str.endswith("CE", na=False))
    )
    mask_pe = (
    ...
    try:
        ...
        if not json_data:
            ...
            socketio.emit(
                ...
                {"status": "error", "message": "Failed to download MStock Master Contract"},
            )
            return
        ...
        if token_df is None or token_df.empty:
            socketio.emit(
                "master_contract_download",
                {"status": "error", "message": "Empty or invalid master contract data"},
            )
            return
