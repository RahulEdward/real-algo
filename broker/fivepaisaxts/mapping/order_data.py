...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        mapped_order_type = order_type_mapping.get(
...
def transform_positions_data(positions_data):
    ...
    if not isinstance(positions_data, list):
        logger.error(
            f"Error: positions_data is not a list. Received: {type(positions_data)} - {positions_data}"
        )
        return transformed_data
...
def map_portfolio_data(portfolio_data):
    ...
    Returns:
    - A dictionary with 'holdings' and 'totalholding' keys structured for the RealAlgoXTS system.
    """
    # Check if response is valid and contains result data
    if (
        not portfolio_data
        or portfolio_data.get("type") != "success"
        or "result" not in portfolio_data
    ):
        logger.info("No data available.")
