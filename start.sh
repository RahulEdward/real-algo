...
if [ -f "$ENV_FILE" ] && [ -r "$ENV_FILE" ] && [ -s "$ENV_FILE" ]; then
    echo "[RealAlgo] Using existing .env file"
else
    echo "[RealAlgo] No .env file found or file is empty. Checking for environment variables..."
    
    # Check if we're on Railway/Cloud (HOST_SERVER is the key indicator)
    if [ -n "$HOST_SERVER" ]; then
        echo "[RealAlgo] Environment variables detected. Generating .env file..."
        ...
        if ! touch "$ENV_FILE" 2>/dev/null; then
            echo "[RealAlgo] Cannot write to /app/.env, using /tmp/.env"
            ENV_FILE="/tmp/.env"
        fi
        ...
        cat > "$ENV_FILE" << EOF
# RealAlgo Environment Configuration File
...
DATABASE_URL = '${DATABASE_URL:-sqlite:///db/realalgo.db}'
...
EOF
        ...
        echo "[RealAlgo] .env file generated at $ENV_FILE"
        echo "[RealAlgo] Configuration: HOST_SERVER=${HOST_SERVER}"
        ...
        if [ "$ENV_FILE" = "/tmp/.env" ]; then
            ...
            echo "[RealAlgo] Linked .env to /app/.env"
        fi
    else
        echo "============================================"
...
# ============================================
echo "[RealAlgo] Starting WebSocket proxy server on port 8765..."
/app/.venv/bin/python -m websocket_proxy.server &
WEBSOCKET_PID=$!
echo "[RealAlgo] WebSocket proxy server started with PID $WEBSOCKET_PID"

# ============================================
# CLEANUP HANDLER
# ============================================
cleanup() {
    echo "[RealAlgo] Shutting down..."
...
# ============================================
# START MAIN APPLICATION (FastAPI with uvicorn)
# ============================================
# Use PORT env var if set (Railway/cloud), otherwise default to 5000
APP_PORT="${PORT:-5000}"

echo "[RealAlgo] Starting FastAPI application on port ${APP_PORT} with uvicorn..."
