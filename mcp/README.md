...
This is a Model Context Protocol (MCP) server that provides trading and market data functionality through the RealAlgo platform. It enables AI assistants to execute trades, manage positions, and retrieve market data directly from supported brokers.

## Prerequisites

### 1. RealAlgo Server Setup

Ensure your RealAlgo server is running and properly configured:

1. **Start RealAlgo Server**: Your RealAlgo server should be running (e.g., on `http://127.0.0.1:5000`)
2. **Verify Connection**: Test that the server is accessible by visiting the web interface.
3. **Broker Authentication**: Ensure your broker credentials are properly configured in RealAlgo.

### 2. API Key

To get your RealAlgo API key:
1. Open your RealAlgo web interface (e.g., `http://127.0.0.1:5000`)
...
```json
{
  "mcpServers": {
    "realalgo": {
      "command": "D:\\realalgo-mcp\\realalgo\\.venv\\Scripts\\python.exe",
      "args": [
        "D:\\realalgo-mcp\\realalgo\\mcp\\mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
```json
{
  "mcpServers": {
    "realalgo": {
      "command": "/Users/your_username/realalgo/.venv/bin/python3",
      "args": [
        "/Users/your_username/realalgo/mcp/mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
```json
{
  "mcpServers": {
    "realalgo": {
      "command": "/home/your_username/realalgo/.venv/bin/python3",
      "args": [
        "/home/your_username/realalgo/mcp/mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
- **Windows**: Replace `D:\\realalgo-zerodha\\realalgo` with your actual RealAlgo installation path
...
### Utilities
- `get_realalgo_version` - Check RealAlgo version
...
1. **Connection Issues**: Verify RealAlgo server is running on `http://127.0.0.1:5000`
...
4. **Order Failures**: Verify broker credentials in RealAlgo are valid and active

## Support

For issues related to:
- **RealAlgo Platform**: Visit the RealAlgo documentation
