...
RealAlgo is a production-ready algorithmic trading platform built with Flask (backend) and React 19 (frontend). It provides a unified API layer across 24+ Indian brokers, enabling seamless integration with TradingView, Amibroker, Excel, Python, and AI agents.

**Repository**: https://github.com/marketcalls/realalgo
...
RealAlgo has TWO frontend systems that coexist:
...
RealAlgo uses **5 separate databases** for isolation:

- `db/realalgo.db` - Main database (users, orders, positions, settings)
...
5. `mapping/` - Transform RealAlgo format ↔ broker format
...
RealAlgo uses standardized symbol format across all brokers:
```
NSE:SBIN-EQ          # Equity
