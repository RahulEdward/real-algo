# routers/pnltracker.py
"""
FastAPI PnL Tracker Router for RealAlgo
Handles intraday PnL tracking and visualization.
Requirements: 4.7
"""
...
pnltracker_router = APIRouter(prefix="", tags=["pnltracker"])
...
def parse_trade_timestamp(timestamp_str, fallback_date=None):
    ...
    formats = [
        "%d-%b-%Y %H:%M:%S", "%H:%M:%S %d-%m-%Y", "%d-%m-%Y %H:%M:%S",
    ...
    if ":" in timestamp_str and " " not in timestamp_str:
        try:
            ...
            if len(time_parts) >= 2 and len(time_parts[0]) <= 2:
                ...
                dt = datetime.combine(
                    today,
                    dt_time(int(time_parts[0]), int(time_parts[1]), int(time_parts[2]) if len(time_parts) > 2 else 0),
                )
                return ist.localize(dt)
...
@pnltracker_router.get("/pnltracker/legacy")
async def pnltracker(request: Request, session: dict = Depends(check_session_validity)):
    """Render the PnL tracker page (legacy Jinja template)."""
    return templates.TemplateResponse("pnltracker.html", {"request": request})


@pnltracker_router.get("/test_chart")
...
@pnltracker_router.post("/pnltracker/api/pnl")
async def get_pnl_data(request: Request, session: dict = Depends(check_session_validity)):
    ...
    try:
        ...
        if not success:
            ...
            return JSONResponse(tradebook_response, status_code=status_code)
            ...
            if success and "data" in positions_response:
                ...
                for pos in positions_data:
                    ...
                    try:
                        ...
                        qty = avg_price = ltp = pnl = 0

                    current_positions[key] = {"quantity": qty, "average_price": avg_price, "ltp": ltp, "pnl": pnl}
        ...
        # Process each symbol's trades and calculate PnL
        for symbol_key, trades_list in symbol_trades.items():
            ...
            for trade in trades_list:
                ...
                if action == "BUY":
                    position_windows.append({
                        "start_time": trade_time, "end_time": None, "qty": qty,
            ...
            try:
                ...
                success, hist_response, _ = get_history(
                    ...
                    start_date=today_str, end_date=today_str, api_key=api_key,
                )
        ...
        # Continue with portfolio calculation...
        return await _calculate_portfolio_pnl(portfolio_pnl, current_positions, first_trade_time, trades, api_key, today_str)
...
async def _calculate_portfolio_pnl(portfolio_pnl, current_positions, first_trade_time, trades, api_key, today_str):
    """Helper function to calculate final portfolio PnL metrics."""
    ist = pytz.timezone("Asia/Kolkata")
    ...
    if portfolio_pnl is None and current_positions:
        ...
        if first_trade_time and trades:
            ...
            if first_trade_time > market_open:
                pre_trade_index = pd.date_range(start=market_open, end=first_trade_time, freq="1min", tz=ist)[:-1]
    ...
    if not portfolio_pnl.empty:
        for idx, row in portfolio_pnl.iterrows():
            try:
                ...
                drawdown_series.append({"time": timestamp_ms, "value": round(float(drawdown_value), 2)})
            except Exception as e:
