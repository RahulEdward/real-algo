# routers/system_permissions.py
"""
FastAPI System Permissions Router for RealAlgo
Checks file and directory permissions for RealAlgo components.
...
system_permissions_router = APIRouter(prefix="/api/system", tags=["system_permissions"])
...
def get_permission_checks():
    ...
    main_db = extract_db_path("DATABASE_URL", "db/realalgo.db")
...
def get_base_path():
    """Get the base path of the RealAlgo application."""
...
def get_windows_permissions(path: str) -> dict:
    ...
    try:
        ...
        executable = os.access(path, os.X_OK) if is_dir else True
        ...
        if perms:
            ...
            if is_sensitive:
                if actual_mode != expected_mode:
                    result["issue"] = (
                        f"Permission should be {format_permission(expected_mode)}, "
                        f"currently {format_permission(actual_mode)}"
                    )
                else:
                    result["is_correct"] = True
            else:
                ...
                if (owner_actual & owner_expected) != owner_expected:
                    result["issue"] = (
                        f"Owner permission should be at least {oct(owner_expected)[2:]}, "
                        f"currently {oct(owner_actual)[2:]}"
                    )
                else:
                    result["is_correct"] = True
...
@system_permissions_router.get("/permissions")
async def get_permissions(request: Request, _: None = Depends(check_session_validity)):
...
@system_permissions_router.post("/permissions/fix")
async def fix_permissions(request: Request, _: None = Depends(check_session_validity)):
