...
**Username Clarification**: The `username` parameter in the API refers to your **RealAlgo login username** (the username you use to login to the RealAlgo application), NOT your Telegram username (@handle).

- ✅ **Correct**: Use `"rajandran"` (your RealAlgo login username)
...
1. **RealAlgo must be running**
   - Start RealAlgo application
   - Ensure it's accessible at `http://127.0.0.1:5000`

2. **Telegram Bot Must Be Running**
   - Go to RealAlgo Telegram settings (`/telegram`)
...
3. **User Must Be Linked**
   ...
   - Username must match your RealAlgo account username

4. **API Key and Username**
   - Get your API key from RealAlgo settings
   - Replace `"your_api_key_here"` in the test files with your actual API key
   - Replace `"your_username_here"` with your **RealAlgo login username** (the username you use to login to RealAlgo app)
   - **IMPORTANT**: Use RealAlgo username, NOT your Telegram username (@handle)
...
cd D:/realalgo-sandbox-test/realalgo/test
...
cd D:/realalgo-sandbox-test/realalgo
...
USERNAME = "your_username_here"      # Replace with your RealAlgo login username (NOT @telegram_handle)
```

**IMPORTANT**: `USERNAME` should be your RealAlgo login username (e.g., "rajandran"), NOT your Telegram username (e.g., "@rajandranr").
...
**Solution**:
1. Go to RealAlgo Telegram settings (`/telegram`)
2. Click "Start Bot"
...
4. **Ensure you're using RealAlgo login username, NOT Telegram username**
   - ✅ Correct: `"rajandran"` (RealAlgo login)
...
**Solution**:
1. Check bot status in RealAlgo dashboard
2. Verify username with `/status` in Telegram bot
3. Check RealAlgo logs for errors
...
- Check bot status in RealAlgo Telegram dashboard
- Verify user linking with `/status` in Telegram
- Review RealAlgo logs for detailed error messages
