...
mkdir -p ~/realalgo-install
cd ~/realalgo-install

# Download the installation script
wget https://raw.githubusercontent.com/marketcalls/realalgo/main/install/install.sh

# Or using curl
curl -O https://raw.githubusercontent.com/marketcalls/realalgo/main/install/install.sh
...
- Set up the RealAlgo application with unique deployment name
...
- Each deployment gets a unique service name (e.g., realalgo-yourdomain-broker)
...
1. **Check Service Status**
   ...
   sudo systemctl status realalgo-fyers-yourdomain-fyers
   
   # Example for Zerodha deployment
   sudo systemctl status realalgo-zerodha-yourdomain-zerodha
   ```
...
2. **Application Not Starting**
   ...
   sudo journalctl -u realalgo-fyers-yourdomain-fyers    # View logs
   sudo systemctl restart realalgo-fyers-yourdomain-fyers # Restart service
   
   # Example 2: Zerodha deployment on zerodha.yourdomain.com
   sudo journalctl -u realalgo-zerodha-yourdomain-zerodha # View logs
   sudo systemctl restart realalgo-zerodha-yourdomain-zerodha # Restart service
   ```
...
1. **SELinux blocking Nginx**
   ...
   sudo semanage fcontext -a -t httpd_sys_rw_content_t "/var/python/realalgo-flask(/.*)?"
   sudo restorecon -Rv /var/python/realalgo-flask
   ```
...
1. **Service Management Examples**
   ```bash
   # List all RealAlgo services
   systemctl list-units "realalgo-*"
   
   # Example outputs:
   # realalgo-fyers-yourdomain-fyers.service    loaded active running
   # realalgo-zerodha-yourdomain-zerodha.service loaded active running
   
   # Restart specific deployment
   sudo systemctl restart realalgo-fyers-yourdomain-fyers
...
2. **Log Management Examples**
   ...
   sudo journalctl -f -u realalgo-fyers-yourdomain-fyers
   
   # View last 100 lines of Zerodha deployment logs
   sudo journalctl -n 100 -u realalgo-zerodha-yourdomain-zerodha
   
   # View logs since last hour for specific deployment
   sudo journalctl --since "1 hour ago" -u realalgo-fyers-yourdomain-fyers
   ```
...
4. **Installation Directory Examples**
   ...
   ls -l /var/python/realalgo-flask/
   
   # Example structure:
   # /var/python/realalgo-flask/fyers-yourdomain-fyers/
   # /var/python/realalgo-flask/zerodha-yourdomain-zerodha/
   
   # Check specific deployment files
   ls -l /var/python/realalgo-flask/fyers-yourdomain-fyers/
   ```
...
2. **SELinux (RHEL-based systems)**
   - The installation script **automatically configures SELinux** for RealAlgo
...
- Check the [GitHub repository](https://github.com/marketcalls/realalgo)
