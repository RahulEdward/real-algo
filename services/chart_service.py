...
def get_chart_preferences(api_key: str) -> tuple[bool, dict[str, Any], int]:
    ...
    Args:
        api_key: RealAlgo API key for authentication
    ...
    if not user_id:
        ...
        return False, {"status": "error", "message": "Invalid realalgo apikey"}, 403
    ...
    return True, {"status": "success", "data": prefs}, 200
...
def update_chart_preferences(
    ...
    if not isinstance(data, dict):
        logger.warning(
            f"[ChartService] update_chart_preferences: Invalid data type: {type(data).__name__}"
        )
        return (
            False,
            {"status": "error", "message": "Invalid data format: expected JSON object"},
            400,
        )

    logger.info(
        f"[ChartService] update_chart_preferences: Saving {len(data)} preferences: {list(data.keys())}"
    )
    ...
    if success:
        ...
        return True, {"status": "success", "message": "Preferences updated successfully"}, 200
    else:
