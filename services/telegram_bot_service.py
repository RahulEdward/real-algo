...
class TelegramBotService:
    """Service class for managing Telegram bot operations with RealAlgo SDK integration"""
    ...
    def __init__(self):
        ...
        self.sdk_clients = {}  # Cache for RealAlgo SDK clients per user
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if telegram_user:
            ...
            await update.message.reply_text(
                f"Welcome to RealAlgo Bot, {user.first_name}! 🚀\n\n"
                "To get started, link your RealAlgo account:\n"
...
*Account Management:*
/link `<api_key> <host_url>` - Link your RealAlgo account
...
"""
    ...
    async def cmd_link(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if test_response and test_response.get("status") == "success":
                ...
                # Get the actual RealAlgo username from the API key
                realalgo_username = None
                try:
                    realalgo_username = get_username_by_apikey(api_key)
                    logger.info(f"API key lookup returned: '{realalgo_username}'")
                except Exception as e:
                ...
                if not realalgo_username and test_response.get("data"):
                    ...
                    if isinstance(data, dict):
                        ...
                        if realalgo_username:
                            logger.info(f"Got username from funds response: {realalgo_username}")

                # Log for debugging
                logger.info(
                    f"Linking Telegram user {user.id} (@{user.username}) with RealAlgo username: '{realalgo_username}'"
                )

                # If we still can't get username, DON'T use telegram username with @
                # Use a proper fallback
                if not realalgo_username:
                    # Try to get from session or use telegram ID
                    realalgo_username = f"user_{user.id}"
                    logger.warning(
                        f"Could not get RealAlgo username, using fallback: {realalgo_username}"
                    )
                else:
                    logger.info(f"Successfully retrieved RealAlgo username: {realalgo_username}")

                create_or_update_telegram_user(
                    telegram_id=user.id,
                    username=realalgo_username,  # Use the actual RealAlgo username
                ...
                logger.info(f"Database updated - Username stored as: {realalgo_username}")
    ...
    async def cmd_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if telegram_user:
            ...
            # Get display name (prefer telegram_username, fallback to realalgo_username)
            display_name = (
                telegram_user.get("telegram_username")
                or telegram_user.get("realalgo_username")
                or "N/A"
            )
            ...
            await update.message.reply_text(
                "❌ No linked account found.\nUse /link to connect your RealAlgo account.",
                parse_mode=ParseMode.MARKDOWN,
            )
    ...
    async def cmd_orderbook(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        client = self._get_sdk_client(user.id)
        if not client:
            await update.message.reply_text("❌ Failed to connect to RealAlgo")
            return
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        client = self._get_sdk_client(user.id)
        if not client:
            await context.bot.send_message(chat_id=chat_id, text="❌ Failed to connect to RealAlgo")
            return
    ...
    async def broadcast_message(self, message: str, filters: dict = None) -> tuple[int, int]:
        ...
        try:
            ...
            if filters:
                ...
                if filters.get("realalgo_username"):
                    users = [
                        u
                        for u in users
                        if u.get("realalgo_username") == filters["realalgo_username"]
                    ]
