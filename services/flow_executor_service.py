...
Executes workflow nodes using internal RealAlgo services (synchronous Flask version)
"""
...
from services.flow_realalgo_client import FlowRealAlgoClient, get_flow_client
...
class NodeExecutor:
    ...
    def __init__(self, client: FlowRealAlgoClient, context: WorkflowContext, logs: list):
        self.client = client
    ...
    def execute_subscribe_ltp(self, node_data: dict) -> dict:
        ...
        Connects to RealAlgo WebSocket server and subscribes to LTP updates.
    ...
    def execute_subscribe_quote(self, node_data: dict) -> dict:
        ...
        Connects to RealAlgo WebSocket and subscribes to quote updates (OHLC + volume).
        Falls back to REST API if WebSocket fails or times out.
        """
        symbol = self.get_str(node_data, "symbol", "")
        exchange = self.get_str(node_data, "exchange", "NSE")
    ...
    def execute_subscribe_depth(self, node_data: dict) -> dict:
        ...
        Connects to RealAlgo WebSocket and subscribes to depth updates (order book).
        Falls back to REST API if WebSocket fails or times out.
        """
        symbol = self.get_str(node_data, "symbol", "")
        exchange = self.get_str(node_data, "exchange", "NSE")
