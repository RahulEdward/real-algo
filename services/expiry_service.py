...
def get_expiry_dates(
    ...
    try:
        ...
        if not symbol or not symbol.strip():
            ...
            return (
                False,
                {"status": "error", "message": "Symbol parameter is required and cannot be empty"},
                400,
            )
        ...
        logger.info(
            f"Getting expiry dates for symbol: {symbol}, exchange: {exchange}, instrumenttype: {instrumenttype}"
        )
        ...
        return (
            False,
            {"status": "error", "message": "An error occurred while fetching expiry dates"},
            500,
        )
