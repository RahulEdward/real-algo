...
This strategy demonstrates how to integrate with RealAlgo API
"""
...
# Get API key from realalgo portal
api_key = os.getenv('REALALGO_APIKEY')

if not api_key:
    print("Error: REALALGO_APIKEY environment variable not set")
    exit(1)


# Set the strategy details and trading parameters
strategy = "EMA Crossover Python"
symbol = 'NHPC'  # RealAlgo Symbol
exchange = "NSE"
product = "MIS"
quantity = 1
...
def ema_strategy():
    ...
    while True:
        try:
            ...
            # Fetch 1-minute historical data using RealAlgo
            df = client.history(
                symbol=symbol,
                exchange=exchange,
                interval="1m",
