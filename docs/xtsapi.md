# ⚙️ How to Integrate Any XTS API-Supported Broker in RealAlgo (5-Minute Setup)

RealAlgo already supports XTS API through the `compositedge` plugin. Any broker using XTS (like IIFL, Nirmal Bang, Anand Rathi, Jainam, 5paisa, etc.) can be added with **zero code duplication**—just a few config updates.
...
│
└── mapping/
    ├── order_data.py           # RealAlgo → XTS order translation
    └── transform_data.py       # XTS → RealAlgo data formatting
```
...
```json
{
  ...
  "Plugin URI": "https://realalgo.in",
  "Description": "CompositedgeRealAlgo Plugin",
  "Version": "1.0",
  "Author": "Kalaivani",
  "Author URI": "https://realalgo.in"
}
```
...
Thanks to RealAlgo’s modular and broker-agnostic design:
