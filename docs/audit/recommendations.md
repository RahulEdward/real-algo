...
When deploying RealAlgo using `install.sh` on Ubuntu server, most security measures are **automatically configured**. This document covers what's already done and what remains for you.
...
```bash
# View RealAlgo logs
sudo journalctl -u realalgo-yourdomain-broker -f
...
# Update RealAlgo dependencies
cd /var/python/realalgo-flask/*/realalgo
...
```bash
# Check logs
sudo journalctl -u realalgo-yourdomain-broker -n 50
...
sudo chown -R www-data:www-data /var/python/realalgo-flask/*/
sudo chmod -R 755 /var/python/realalgo-flask/*/
```
...
4. **Review logs**:
   ```bash
   sudo journalctl -u realalgo-* --since "24 hours ago"
