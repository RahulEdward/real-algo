...
RealAlgo's React frontend supports theme customization with light/dark modes and 8 accent colors. Theme preferences persist across sessions using Zustand with localStorage.
...
function useSystemTheme() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('realalgo-theme');
