...
The PlaceOrder API is the core order execution endpoint in RealAlgo. It handles order validation, authentication, broker routing, and response processing through multiple layers.
...
|-------|------|-------------|
| apikey | string | RealAlgo API key |
...
| Invalid API key | 403 | `{"status": "error", "message": "Invalid realalgo apikey"}` |
