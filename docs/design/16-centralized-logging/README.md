...
RealAlgo implements centralized logging with configurable levels, file rotation, and structured output. All application logs are routed through a unified logging system stored in `logs.db` and optional file logs.
    ...
    if not logger.handlers:
        ...
        if os.getenv('LOG_TO_FILE', 'False').lower() == 'true':
            file_handler = RotatingFileHandler(
                filename=os.path.join(os.getenv('LOG_DIR', 'log'), 'realalgo.log'),
...
```
log/
├── realalgo.log        # Current log file
├── realalgo.log.1      # Previous rotation
├── realalgo.log.2      # Older rotation
├── ...
└── realalgo.log.14     # Oldest (based on LOG_RETENTION)
```
...
```bash
# View current log
cat log/realalgo.log

# Follow log in real-time
tail -f log/realalgo.log

# View last 100 lines
tail -100 log/realalgo.log

# Search for errors
grep ERROR log/realalgo.log
```
