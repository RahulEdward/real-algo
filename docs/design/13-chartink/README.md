...
Chartink integration allows RealAlgo to receive trading signals from Chartink screener alerts via webhooks. When a stock appears in a Chartink scanner, it triggers a webhook that RealAlgo processes to place trades automatically.
...
class ChartinkSymbolMapping(Base):
    ...
    symbol = Column(String(50))               # RealAlgo symbol
...
```json
{
    "webhook_id": "your_webhook_id_from_realalgo",
    "stocks": "{stocks}"
}
```
...
Map Chartink symbols to RealAlgo format:

| Chartink Symbol | RealAlgo Symbol | Exchange | Product | Quantity |
|-----------------|-----------------|----------|---------|----------|
...
RealAlgo: Auto-buy with MIS product, qty=100
```
...
RealAlgo: Auto-buy with CNC product for delivery
```
...
RealAlgo: Auto-sell to close positions
```
