...
RealAlgo provides Docker support for containerized deployment with multi-stage builds, IST timezone configuration, and proper security isolation. The Docker setup uses Python 3.12, Gunicorn with Eventlet workers, and runs as a non-root user.
...
# View logs
docker logs -f realalgo

# Stop container
docker stop realalgo
