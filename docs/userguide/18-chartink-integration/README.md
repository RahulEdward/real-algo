...
ChartInk is a powerful stock screening platform popular among Indian traders. It can scan thousands of stocks in real-time and send webhook alerts when conditions are met. RealAlgo integrates with ChartInk to execute trades automatically based on your screener alerts.

## How It Works

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ChartInk → RealAlgo Flow                                 │
│                                                                              │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌──────────┐  │
│  │  ChartInk   │     │   Webhook   │     │  RealAlgo   │     │  Broker  │  │
...
2. RealAlgo running and accessible via internet
3. API key generated in RealAlgo
4. Broker connected and logged in

## Making RealAlgo Accessible for Webhooks

ChartInk webhooks need to reach your RealAlgo server from the internet.

### Recommended: Production Server with Domain

Deploy RealAlgo on an Ubuntu server using `install.sh` (see [Installation Guide](../04-installation/README.md)):
...
**Important**: Tunnel services are **only for webhooks**, not for running the full application. Always run RealAlgo on your own server for production use
...
2. Enable **Webhook**
3. Enter your RealAlgo webhook URL
...
```
https://your-realalgo-url/api/v1/placeorder
```

### ChartInk Webhook Payload

ChartInk sends data in a specific format. You need to configure the payload to match RealAlgo's API:
...
1. Enable Analyzer Mode in RealAlgo
...
- Check RealAlgo dashboard regularly
- Set up Telegram notifications
- Review trades daily

## Troubleshooting

### Alert Not Reaching RealAlgo
...
1. Go to **Traffic Logs** in RealAlgo
