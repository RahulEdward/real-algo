...
This guide walks you through installing RealAlgo on your system. We'll cover Windows, Ubuntu, and macOS installations.
...
#### Step 3: Download RealAlgo

Open Command Prompt (search "cmd") and run:

```cmd
# Navigate to where you want RealAlgo
...
#### Step 3: Download and Run RealAlgo
...
You should see the RealAlgo login page.
...
For production use, deploy RealAlgo on an Ubuntu server using the automated `install.sh` script. This is the **recommended approach** for live trading.
...
- Unique service name (e.g., realalgo-yourdomain-broker)
...
1. **Check Service Status**
   ```bash
   sudo systemctl status realalgo-yourdomain-broker
   ```
...
sudo systemctl restart realalgo-yourdomain-broker

# View real-time logs
sudo journalctl -f -u realalgo-yourdomain-broker

# View last 100 lines of logs
sudo journalctl -n 100 -u realalgo-yourdomain-broker
```
...
```bash
# View service logs
sudo journalctl -u realalgo-yourdomain-broker
...
**Important**: Tunneling is **only for webhooks**. Always run RealAlgo on your own server with proper domain setup for production use. Don't run the entire application through a tunnel.
...
RealAlgo can also be deployed using Docker with custom domain and SSL. This is useful if you prefer containerized deployments.
...
5. Clones RealAlgo to `/opt/realalgo`
...
RealAlgo can run on Raspberry Pi models 3, 4, or 5 (4GB+ RAM), preferably with Ubuntu 24.04+ server edition.
...
**Clone and Build:**
```bash
git clone https://github.com/marketcalls/realalgo
cd realalgo
cp .sample.env .env
# Edit .env with your broker credentials
docker build -t realalgo:latest .
docker-compose up -d
```
...
Close all RealAlgo instances and restart.
