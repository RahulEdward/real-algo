...
Before diving into RealAlgo, let's understand the key terms and concepts you'll encounter. This foundation will make everything else easier to understand.
...
**In RealAlgo**: When TradingView wants to place an order, it sends a request to RealAlgo's API. RealAlgo processes it and sends the order to your broker.

```
TradingView → "Place BUY order for SBIN" → RealAlgo API → Broker
```

### 2. API Key

**Simple Explanation**: Your API key is like a password that identifies you. It proves to RealAlgo that the request is coming from an authorized source.
...
**Simple Explanation**: A webhook is like a doorbell. When something happens (like a TradingView alert), it "rings" your RealAlgo server to notify it.
...
Webhook sends data to your URL
        ↓
RealAlgo receives and processes
        ↓
...
**Simple Explanation**: When you log into your broker through RealAlgo, you get a temporary pass (token) that lets RealAlgo place orders on your behalf.
...
**Simple Explanation**: Every stock has a specific way to write its name that RealAlgo understands.

**Examples**:
| What you want | RealAlgo Symbol |
|---------------|-----------------|
...
**Simple Explanation**: The period you're logged into RealAlgo. Sessions expire for security.

**Browser Session**: Your RealAlgo web login
...
**Previous**: [02 - Why Build with RealAlgo](../02-why-build-with-realalgo/README.md)
